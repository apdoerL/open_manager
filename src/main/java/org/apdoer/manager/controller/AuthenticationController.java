...
import org.apdoer.manager.model.vo.AuthenticationVo;
import org.apdoer.manager.model.vo.AuthorizationUserVo;
import org.apdoer.manager.model.vo.ImgVo;
...
public class AuthenticationController {
    ...
    public ResponseEntity login(@Validated @RequestBody AuthorizationUserVo authorizationUser){
        ...
        // 返回 token
        return ResponseEntity.ok(new AuthenticationVo(token,jwtUser));
    }
    ...
    public ImgVo getCode(HttpServletResponse response) throws IOException {
        ...
        try {
            return new ImgVo(Base64.encode(stream.toByteArray()),uuid);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        } finally {
