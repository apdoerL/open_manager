...
import org.apdoer.manager.repository.QiNiuConfigRepository;
import org.apdoer.manager.repository.QiniuContentRepository;
...
public class QiNiuHandlerImpl implements QiNiuHandler {
    ...
    public Object queryAll(QiniuQueryCriteria criteria, Pageable pageable){
        return PageUtil.toPage(qiniuContentRepository.findAll((root, criteriaQuery, criteriaBuilder) -> QueryHelper.getPredicate(root,criteria,criteriaBuilder),pageable));
    }

    @Override
    public QiniuCloudPo find() {
        Optional<QiniuCloudPo> qiniuConfig = qiNiuConfigRepository.findById(1L);
        if(qiniuConfig.isPresent()){
            return qiniuConfig.get();
        } else {
            return new QiniuCloudPo();
        }
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public QiniuCloudPo update(QiniuCloudPo qiniuConfig) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public QiniuFilePo upload(MultipartFile file, QiniuCloudPo qiniuConfig) {
        ...
        try {
            ...
            //存入数据库
            QiniuFilePo qiniuContent = new QiniuFilePo();
    ...
    @Override
    public QiniuFilePo findByContentId(Long id) {
        Optional<QiniuFilePo> qiniuContent = qiniuContentRepository.findById(id);
        ValidationUtil.isNull(qiniuContent,"QiniuContent", "id",id);
        return qiniuContent.get();
    }

    @Override
    public String download(QiniuFilePo content,QiniuCloudPo config){
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void delete(QiniuFilePo content, QiniuCloudPo config) {
        //构造一个带指定Zone对象的配置类
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void synchronize(QiniuCloudPo config) {
    ...
    @Override
    public void deleteAll(Long[] ids, QiniuCloudPo config) {
        for (Long id : ids) {
